...
export async function POST() {
  try {
    const account = await prisma.gmailAccount.findUnique({
      where: { email: AE_EMAIL },
    });

    if (!account?.refreshToken) {
      return NextResponse.json(
        { error: `Gmail not connected for ${AE_EMAIL}` },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (msg.includes("insufficient") || msg.includes("scope") || msg.includes("forbidden") || e?.status === 403) {
        return NextResponse.json(
          { error: "Gmail account needs read permission. Click Re-connect to re-authorize, then try again." },
          { status: 400 }
        );
      }
      throw e;
    ...
    const AE_LOWER = AE_EMAIL.toLowerCase();
    ...
    for (const msg of messages) {
      try {
        ...
        const found = extractBouncedEmails(full.data).filter((e) => e !== AE_LOWER);
        for (const e of found) bouncedEmails.add(e);

        const subject = full.data.payload?.headers?.find(
          (h: any) => h.name?.toLowerCase() === "subject"
        )?.value ?? "(no subject)";
        debugMessages.push({ id: msg.id!, subject, snippet: full.data.snippet ?? "", extracted: found });
      } catch {
    ...
    const result = await prisma.contact.updateMany({
      where: { email: { in: [...bouncedEmails] }, status: "active" },
      data: { status: "bounced" },
    });
