...
export default function AeCampaignsPage() {
  ...
  const [name, setName] = useState("AE Monthly Project Invites");
  ...
  async function loadAll() {
    ...
    try {
      ...
      if (ac) {
        setName(ac.name || "AE Monthly Project Invites");
        ...
        setName("AE Monthly Project Invites");
        setActive(true);
      ...
      setError("Failed to load AE campaign data");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function save() {
    ...
    const payload = {
      id: autoCampaign?.id,
      name: name.trim() || "AE Monthly Project Invites",
    ...
    try {
      const res = await fetch("/api/ae-campaigns/upsert", {
        method: "POST",
  ...
  async function resetSendHistory() {
    ...
    setOkMsg(null);
    try {
      const res = await fetch(`/api/ae-campaigns/${autoCampaign.id}/reset`, { method: "POST" });
      const data = await res.json();
  ...
  async function toggleActive() {
    ...
    try {
      const res = await fetch(`/api/ae-campaigns/${autoCampaign.id}/toggle`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function sendTest() {
    ...
    try {
      const res = await fetch(`/api/ae-campaigns/${autoCampaign.id}/test-send`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        <section className="lg:col-span-5 space-y-4">
            ...
            <div className="mt-4 space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="AE Monthly Project Invites"
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="ae-active" className="text-sm text-slate-700">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-1 text-xs text-slate-500">
              ...
              <code className="rounded bg-slate-100 px-1">{"{{project}}"}</code> per email.
            </p>
            <textarea
