...
export async function scanBouncesForPoolAccounts(): Promise<{
  accounts: { email: string; scanned: number; bounced: number; error?: string }[];
  totalBouncesFound: number;
  contactsMarked: number;
}> {
  const accounts = await prisma.gmailAccount.findMany({ where: { usedForMass: true } });
  ...
  return { accounts: accountSummary, totalBouncesFound: allBounced.size, contactsMarked };
}
